...
st.set_page_config(page_title="Risk Mapping · RegWatch", page_icon="🗺️", layout="wide")
...
with st.sidebar:
    ...
    if impact:
        ...
        st.success(f"Impact Analysis ✓ — {len(impacted)} impacted · {len(non_impacted)} non impacted")
        if impacted:
        ...
        st.warning("No Impact Analysis results.\nRun Impact Analysis in Product Mode first.")

    if agent5a:
        st.success(f"Legal Sheet ✓ — {len(agent5a)} section(s) approved")
    else:
        st.info("Legal Sheet: no updates\n(before/after comparison disabled)")

    st.divider()
    st.caption("Workflow: Regulatory Watch → Impact Analysis (Product Mode) → Risk Mapping")
    st.divider()
...
with col1:
    st.markdown("**Impact Analysis — Product impact**")
    if impact:
        ...
        with st.expander("Preview"):
            ...
            if len(impacted_) > 3:
                st.caption(f"... and {len(impacted_)-3} more impacted")
    else:
        all_products = []
        st.error("Missing — run Impact Analysis in Product Mode")

with col2:
    st.markdown("**Legal Sheet — Approved updates**")
    if agent5a:
    ...
    if not agent5a:
        uploaded = st.file_uploader(
            "Import Legal Sheet JSON (optional)",
            type=["json"],
            key="5b_import_5a"
        )
        if uploaded:
...
analysis_mode = st.radio(
    ...
    [
        "📡 Regulatory alerts (Impact Analysis output)",
        "📚 Updated legal sheet (Legal Sheet Update — from library)",
    ],
    horizontal=True,
    key="5b_analysis_mode",
    help=(
        "**Alerts mode**: risk is assessed against raw regulatory alerts from Impact Analysis.\n\n"
        "**Legal sheet mode**: risk is assessed against the legal sheet AFTER Legal Sheet Update approved updates — "
...
with col_i:
    if not impact:
        st.warning("Run Impact Analysis in Product Mode first.")
    else:
...
if "5b_result" in st.session_state:
    ...
    with tab_reg:
        ...
        for reg in reg_view:
            ...
            with st.expander(
                f"{icon_u} **{reg.get('regulation', '')}** — {n_affected} product(s) affected",
                expanded=(urg == "HIGH")
            ):
