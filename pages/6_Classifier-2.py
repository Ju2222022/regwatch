...
# ── Header ────────────────────────────────────────────────────────────────────
st.title("🏷️ Regulatory Classifier")
st.caption("Fetches product specs from the web, then classifies into CAT1–CAT9 against Decathlon's regulatory framework.")
st.info("**Workflow:** 🔍 Product Profiler fetches specs by model code → 🏷️ Classifier assigns regulatory categories")
st.divider()
...
if mode == "Single product":
    ...
    if submitted:
        if not model_code.strip():
            ...
            with st.spinner(f"🔍 Profiler — Scraping **{domain.strip()}** for **{model_code.strip()}**…"):
            ...
            # Keep extra_info concise — description capped at 300 chars to control cost
            extra = classifier_input.get("extra_info", "")[:500]
            desc  = classifier_input.get("description", "")[:300]
            if desc:
...
if "last_classification" in st.session_state:
    ...
    if _cats:
        ...
        with st.container(border=True):
            ...
            with col_send:
                st.markdown("**➡️ Ready for Impact Analysis**")
                if st.button("📊 Send to Impact Analysis", type="primary", key="send_to_impact_from_cls"):
                    st.switch_page("pages/2_Impact.py")
            with col_info:
                st.caption(f"Classification **{', '.join(_cats)}** ready — Impact Analysis will assess regulatory impact on your catalog.")
