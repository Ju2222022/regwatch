...
st.title("📊 Concordance Report — Agent 3 vs Manual Classifications")
st.caption("Compare AI classifications with your reference classifications")

uploaded = st.file_uploader(
    "File with manual classifications (columns: code, name, type, ground_truth)",
    type=["csv", "xlsx"]
)

CAT_LABELS = {
    "CAT1": "Batteries", "CAT2": "Lamps", "CAT3": "Electronics (base)",
...
st.subheader("PoC Results — 11 reference products")
...
for r in POC_RESULTS:
    ...
    if ai == gt:
        ...
        status = "🟡 Over-classified"
    elif not extra:
        status = "🟠 Under-classified"
    else:
        status = "❌ Divergence"
    rows.append({
        "Product": r["name"],
        ...
        "Missing (AI)": ", ".join(sorted(missing)) if missing else "—",
...
# Métriques globales
perfect = sum(1 for r in rows if r["Status"] == "✅ Perfect")
total = len(rows)
missing_total = sum(1 for r in rows if r["Missing (AI)"] != "—")

col1, col2, col3, col4 = st.columns(4)
col1.metric("Perfect match", f"{perfect}/{total}", f"{perfect/total*100:.0f}%")
col2.metric("No missing category", f"{total - missing_total}/{total}")
col3.metric("Total divergence", "0/11", "✅")
col4.metric("Prompt version", "v2 — Parachute")

st.info("💡 Remaining gaps (HL50 CAT3, DS100 CAT9, FIT100M CAT7) are open business questions — to be decided with the regulatory team.")
