...
with tab_sources:
    ...
    if markets:
        ...
        for col, market in zip(cols, markets):
            ...
            col.metric(market, f"{len(sources.get(market, []))} domain(s)",
                       f"{flag} {LANGUAGE_LABELS.get(lang, lang)}")

    st.divider()
    tab_labels = markets + ["➕ New market"]
    tabs_m = st.tabs(tab_labels)

    for tab, market in zip(tabs_m[:-1], markets):
        with tab:
...
with tab_ref:
    ...
    with st.expander("➕ Add a new sub-category", expanded=False):
        with st.form("form_add_sub"):
            ...
            new_rule  = st.text_area("Classification rule", height=60)
    ...
    with st.expander("➕ Add a new legal category (new domain)", expanded=False):
        with st.form("form_add_legal_cat"):
            ...
            nc_fallback_rule = st.text_area("Fallback rule", height=60)
            nc_spec_rule     = st.text_area("Specificity rule", height=60)
