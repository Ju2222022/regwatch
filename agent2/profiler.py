...
def profile_product(model_code: str, domain: str = "decathlon.fr",
    ...
    name_hint: optional product name to improve search quality.
    """
    name_hint = name_hint or model_code  # use name if provided, else code
...
def profile_batch(model_codes: list, domain: str = "decathlon.fr",
    ...
    for i, code in enumerate(model_codes):
        ...
        name = (name_hints or {}).get(code.strip(), "")
        results.append(profile_product(code.strip(), domain, jina_key, api_key, tavily_key, name))
    return results
